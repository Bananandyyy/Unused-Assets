...
import com.gizmo.unusedassets.entity.ai.goal.TropicalSlimeAttackGoal;
import com.gizmo.unusedassets.entity.ai.goal.TropicalSlimeFaceRandomGoal;
import com.gizmo.unusedassets.entity.ai.goal.TropicalSlimeFloatGoal;
import com.gizmo.unusedassets.entity.ai.goal.TropicalSlimeHopGoal;
	...
	public ActionResultType func_230254_b_(PlayerEntity player, Hand hand) {
		...
		if (itemstack.getItem() == Items.BUCKET && !player.abilities.isCreativeMode && !this.isChild()) {
			if (!this.world.isRemote) {
				this.world.addParticle(ParticleTypes.EXPLOSION, this.getPosX(), this.getPosYHeight(0.5D),
						this.getPosZ(), 0.0D, 0.0D, 0.0D);
				this.remove();
	...
	public void tick() {
		...
		if (this.onGround && !this.wasOnGround) {
			...
			for (int j = 0; j < i * 8; ++j) {
				...
				this.world.addParticle(this.getSquishParticle(), this.getPosX() + (double) f2, this.getPosY(),
