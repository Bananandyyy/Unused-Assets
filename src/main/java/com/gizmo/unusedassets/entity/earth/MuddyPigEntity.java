...
public class MuddyPigEntity extends PigBase<MuddyPigEntity> {
	private static final DataParameter<Boolean> MUDDY_STATE = EntityDataManager.createKey(MuddyPigEntity.class,
	...
	public void livingTick() {
		...
		boolean condition = this.world.getFluidState(blockPos).getBlockState().getBlock().equals(EarthBlocks.MUD_BLOCK);
		if (condition) {
			if (!isInMuddyState()) {
				if (!isShaking) {
					this.isShaking = true;
